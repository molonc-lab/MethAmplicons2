...
def main():
    ...
    if hasattr(sys, 'real_prefix') or (hasattr(sys, 'base_prefix') and sys.base_prefix != sys.prefix):
        #print(f"The system prefix/virtual environment is {sys.prefix}")
        ...
        #print(f"Will place flash in user's local bin at: {destination_path}")

    if not os.path.exists(flash_path):
        raise Exception("No flash found in user's bin, please download flash binary for your system from https://ccb.jhu.edu/software/FLASH/ then uncompress the file and move it to your bin")
    else:
